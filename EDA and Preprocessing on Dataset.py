...
try:
    ...
    df = pd.read_csv(r"E:/Project 234/Project_cinema_retail_operation/cinema_merged_data_updated.csv")
...
df.isnull().sum()
df = df.drop(columns=['Amount','Customer_Name','Occupation','Income','Transaction_Date_y','Snacks_Purchased','Transaction_Date_x'])
df['Amount'] = df['Ticket_Price']*df['Quantity'] + df['Snack_Amount'] - df['Discount_Applied']


directory = "E:/Project 234/Project_cinema_retail_operation"

#Create the directory if it doesn't exist
os.makedirs(directory, exist_ok=True)

#Save the DataFrame to CSV
file_path = os.path.join(directory, 'cinema_merged_data_modifiedcolumns.csv')
...
plt.boxplot([df['Ticket_Price'], df['Amount'], df['Quantity'], df['Discount_Applied'], 
             df['Snack_Amount'], df['Loyalty_Points']], 
            labels=['Ticket_Price', 'Amount', 'Quantity', 'Discount_Applied', 
                    'Snack_Amount', 'Loyalty_Points'], patch_artist=True)
...
filepath = "E:/Project 234/encoded_dataset.csv"
...
with open('hierarchical_clustering_labels.pkl', 'wb') as file:
